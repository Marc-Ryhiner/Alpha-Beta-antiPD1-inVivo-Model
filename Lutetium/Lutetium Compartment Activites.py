...
def simulate_compartment_activities(ci0, A0, alpha, S_i, p):
    y0 = [ci0, 0, 0, A0, 0, 1]
    ...
    # Combine concentrations and compute activities
    ci_total = np.concatenate((sol1.y[0], sol2.y[0]))
    cb_total = np.concatenate((sol1.y[1], sol2.y[1]))
    cc_total = np.concatenate((sol1.y[2], sol2.y[2]))
    A_total = np.concatenate((sol1.y[3], sol2.y[3]))

    c_total = ci_total + cb_total + cc_total
    interstitial = A_total * ci_total / c_total
    membrane = A_total * cb_total / c_total
    cytoplasm = A_total * cc_total / c_total

    return t_total, interstitial, membrane, cytoplasm
...
# Run simulation
time, interstitial, membrane, cytoplasm = simulate_compartment_activities(ci0, A0, alpha, S_i, base_params)

# Plotting
plt.figure(figsize=(8, 6))
plt.plot(time, interstitial / 1e6, label='Interstitial', color='royalblue')
plt.plot(time, membrane / 1e6, label='Membrane-bound', color='darkorange')
plt.plot(time, cytoplasm / 1e6, label='Cytoplasmic', color='seagreen')
...
plt.title('Compartment- and LET-specific Activities during Lu-177 Treatment for $A_0=2 MBq$')
